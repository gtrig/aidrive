...
TRAINING_SENSOR_LAYOUT = [(-90, 50), (-45, 100), (-20, 150), (0, 200), (20, 150), (45, 100), (90, 50)]
...
ACTIONS = [
    ...
    (0.0,   0),   # 1 – coast
    (-0.1,  0),   # 2 – brake
    (0.05, -3),   # 3 – throttle + left
    (0.05, +3),   # 4 – throttle + right
    (0.05, -6),   # 5 – throttle + sharp left
    (0.05, +6),   # 6 – throttle + sharp right
]

# Reward weights
R_GATE        =  2.0    # crossing the next gate forward
...
R_OFF_ROAD    = -1.0    # episode-ending penalty
R_TIME        = -0.001  # small per-step time pressure
...
class CarEnv:
    ...
    def __init__(
        ...
        gates_file = Path(gates_path) if gates_path else project_root / 'gates.npy'
        ...
        self.track = Track(headless=True)
        ...
        self._prev_pos    = (START_X, START_Y)
        self._prev_dist   = 0.0
    ...
    def reset(self):
        self.car.x          = START_X
        self.car.y          = START_Y
        self.car.speed      = 1.5   # start with some speed so gate 0 is reachable sooner
        self.car.orientation = START_HEADING
